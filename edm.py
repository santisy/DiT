...
def edm_sampler(
    net, latents, model_kwargs=None, randn_like=torch.randn_like,
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Euler step.
        denoised = net(x_hat, t_hat, **model_kwargs).to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            denoised = net(x_next, t_next, **model_kwargs).to(torch.float64)
...
class EDMLoss:
    ...
    def __call__(self, net, inputs, model_kwargs, augment_pipe=None):
        ...
        D_yn = net(y + n, sigma, model_kwargs)
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, model_kwargs, force_fp32=False):
